...
transformed data {
  ...
  x_std[1:NN, 1] = x[1:NN, 1];
  for (m in 2:M) {
    mu_x[m] = mean(x[1:NN, m]);
    sd_x[m] = sd(x[1:NN, m]);
    x_std[1:NN, m] = (x[1:NN, m] - mu_x[m]) / sd_x[m];
  }
  xx_std[1:NN, 1] = xx[1:NN, 1];
  for (m in 2:MM) {
    mu_xx[m] = mean(xx[1:NN, m]);
    sd_xx[m] = sd(xx[1:NN, m]);
    xx_std[1:NN, m] = (xx[1:NN, m] - mu_xx[m]) / sd_xx[m];
  }
}
