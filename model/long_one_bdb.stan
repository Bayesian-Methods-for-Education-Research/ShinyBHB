...
model {
  ...
  for (n in 1:N)
    u_std[n] ~ multi_normal_prec(rep_vector(0, K1), prec_u_std[cycle[n]]);
  {
  ...
  for (s in 1:S) {
    y_std[s] ~ normal(dot_product(x_std[, s], beta_std[cycle[id[s]]]) +
...
generated quantities {
  ...
  for (s in 1:SS) {
    int n = id[s];
    log_lik[n] += normal_lpdf(y[s] | dot_product(x_std[, s], beta_std[cycle[n]]) +
