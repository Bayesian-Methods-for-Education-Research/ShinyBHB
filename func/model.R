# model.R askes users to customize the model
...
# longitudinal layers are handled with another layer
...
# update the model
observeEvent(c(input$long, input$t1, input$td1, input$tf1, input$y1, input$x1, input$x2, input$add_2, input$add_3, input$delete_2, input$delete_3), {
...
observeEvent(c(input$long, input$t1, input$td1, input$tf1, input$y1, input$x1, input$x2, input$select_method, input$select_mode, input$add_2, input$add_3, input$delete_2, input$delete_3), {
    ...
    updateHyperprior <- function(component, label) {
        if (component == "sigma_beta" & model_mode_BDB() == "others") {
            ...
            if (component == 'sigma_0_beta') {
                default_value = 3
            }
            else if (component == 'sigma_beta') {
                default_value = 2
            }
            else if (component == 'Omega' || component == 'Omega_2') {
                default_value = 2
            }
            else {
                default_value = 1
            ...
            tagList(
                if (label == "") {
                    ...
                    div(
                        ...
                        div(style="display:inline-block", class="hyperprior_input", numericInput(inputId=paste0(component, "_b"), label="$b$", value = default_value, width = "30%"))
                    )
                },
                ...
                tagList(
                    if (label == "") {
                        ...
                        div(
                            ...
                            div(style="display:inline-block", class="hyperprior_input", numericInput(inputId=paste0(component, "_b"), label=paste0("$", hyperprior[[paste0(component, "_b")]][input[[paste0('dist_', component)]]], "$"), value = default_value, width = "100px")),
